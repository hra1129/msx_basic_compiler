...
_pt0:
        ...
        ADD         HL, DE					; HL = (I AND 7) * 4 + svarf_on_strig0_mode
        LD          [HL], 1					; ON
        CALL        interrupt_prcess
        LD          HL, [svari_I_LABEL]
        CALL        jp_hl
...
_skip_on_interval:
		...
		; 非割り込みルーチンの中の ON STRIG 処理
        LD          HL, svarf_on_strig0_mode
        LD          DE, svari_on_strig0_line
        LD          B, 5
_on_strig_loop1:
        LD          A, [HL]						; A = svarf_on_strig0_mode[+00]  0: OFF/STOP, 1: ON
        INC         HL							; HL = +01
        DEC         A							; A = FFh:OFF/STOP  0:ON
        JR          NZ, _skip_strig1			; if OFF/STOP then _skip_strig1
        OR          A, [HL]						; ここに来る時点で A = 0 なので、A = svarf_on_strig0_mode[+01] の意味。かつ、その Zero評価付き。
        JR          Z, _skip_strig1				; +01 は、近々の GTTRIG() の値。Z なら、ボタン押されてない。
        INC         HL							; ここに来る時点で A = FFh。
        INC			A							; A = 0
        OR          A, [HL]						; A = svarf_on_strig0_mode[+02] の意味。かつ、その Zero評価付き。
        DEC         HL
        JR          NZ, _skip_strig1
        DEC         A							; A = 0FFh
        LD          [HL], A						
