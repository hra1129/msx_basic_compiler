...
line_100:
        ...
        CALL        interrupt_process
		; DIM A$(4)
        LD          HL, [varsa_A]					; 配列変数 A$() に格納されてるポインタを取得
        LD          A, L
        OR          A, H
        JP          NZ, bios_errhand_redim			; 0000h でなければ、既に確保済みなので redimention error
		; 要素数指定 4
        LD          HL, 4
        INC         HL								; 実際の要素数は +1
        PUSH        HL								; スタックに要素数を保存
        ADD         HL, HL							; 1要素 2byte (ポインタ) なので 要素数を 2倍する
        LD          DE, 5							; ヘッダサイズ 5byte: 内訳 サイズ 2byte, 次元数1byte, 1時限目の要素数 2byte
        ADD         HL, DE							; これで確保したいサイズになる
        PUSH        HL								; 確保サイズを一旦保存
        LD          C, L
        LD          B, H
        CALL        allocate_heap					; メモリを確保
        LD          [varsa_A], HL					; 確保したアドレスを、A$() にセット
        POP         BC								; サイズを復帰
        DEC         BC								; ヘッダの「サイズ 2byte」を除去
        DEC         BC
        LD          [HL], C							; ヘッダの「サイズ 2byte」に、「サイズ 2byte」を除いたサイズを格納
        INC         HL
        LD          [HL], B
        INC         HL
        ...
        LD          [HL], B							; ヘッダの「次元数 1byte」に、1 を格納
        INC         HL
        POP         BC								; スタックから要素数を復帰
        LD          [HL], C							; 第1次元の要素数としてヘッダに格納
        INC         HL
        LD          [HL], B
		inc			hl
