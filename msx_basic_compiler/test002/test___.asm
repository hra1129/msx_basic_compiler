...
_GET_STRING_LOOP_EXIT:
        ...
        PUSH        DE								; 入力文字列のアドレスを保存
		; 文字列領域を確保
        CALL        ALLOCATE_STRING
        POP         DE								; 入力文字列のアドレスを復帰
        LD          B, 0
        PUSH        HL								; 確保したメモリのアドレスを保存
        INC         HL
        EX          DE, HL
        LDIR        								; 入力した文字列を転送
        POP         BC								; 確保したメモリのアドレスを復帰
...
_SUB_INPUT_COPY_STRING:
        ...
        INC         HL
		LD			D, [HL]							; ★追加
        LD          [HL], B							; 変数に確保したメモリアドレスをセット
		LD			HL, SVARS_INPUT_FREE_STR0		; ★追加
_SUB_INPUT_FREE_STRING:								; ★追加
		LD			A, [HL]							; ★追加
		INC			HL								; ★追加
		OR			A, [HL]							; ★追加
		JR			Z, _SUB_INPUT_SET_FREE_STRING	; ★追加
		INC			HL								; ★追加
		JR			_SUB_INPUT_FREE_STRING			; ★追加

_SUB_INPUT_SET_FREE_STRING:							; ★追加
		LD			[HL], D							; ★追加
		DEC			HL								; ★追加
		LD			[HL], E							; ★追加
		POP			HL								; ★追加
