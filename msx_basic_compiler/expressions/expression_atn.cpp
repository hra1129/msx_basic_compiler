...
// --------------------------------------------------------------------
void CEXPRESSION_ATN::optimization( CCOMPILE_INFO *p_this ) {

	this->p_operand->optimization( p_this );
}

// --------------------------------------------------------------------
void CEXPRESSION_ATN::compile( CCOMPILE_INFO *p_this ) {
	...
	p_this->assembler_list.add_label( "bios_atn", "0x02A14" );
	if( this->p_operand->type == CEXPRESSION_TYPE::INTEGER ) {
		...
		p_this->assembler_list.body.push_back( asm_line );
		asm_line.set( CMNEMONIC_TYPE::CALL, CCONDITION::NONE, COPERAND_TYPE::LABEL, "bios_atn", COPERAND_TYPE::NONE, "" );
		p_this->assembler_list.body.push_back( asm_line );
	}
