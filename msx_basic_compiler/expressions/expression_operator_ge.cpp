...
void CEXPRESSION_OPERATOR_GE::compile( CCOMPILE_INFO *p_info ) {
	...
	if( compare_type == CEXPRESSION_TYPE::INTEGER ) {
		...
		p_info->assembler_list.body.push_back( asm_line );
		asm_line.set( CMNEMONIC_TYPE::SBC, CCONDITION::NONE, COPERAND_TYPE::REGISTER, "HL", COPERAND_TYPE::REGISTER, "DE" );	//	HL(ç∂çÄ) >= DE(âEçÄ) Ç™ê¨óßÇ∑ÇÈèÍçáÇÕ S=0 (P), ÇµÇ»Ç¢èÍçáÇÕ S=1 (M)
		p_info->assembler_list.body.push_back( asm_line );
		...
		p_info->assembler_list.body.push_back( asm_line );
		//	A= 1 Å® ARG < DAC : A =-1 Ç…ÇµÇΩÇ¢
		//	A= 0 Å® ARG = DAC : A =-1 Ç…ÇµÇΩÇ¢
		//	A=-1 Å® ARG > DAC : A = 0 Ç…ÇµÇΩÇ¢
